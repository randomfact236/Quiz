...
| SHARE-01  | Share rows: 10/10 LIVE - #8 image riddle, #10 games, deep-links done + live-verified 2026-09-21 | share / OG designs        | P2       | Fixed             |
...
**#8 image riddle - DONE + LIVE-VERIFIED 2026-09-21.** All 5 steps shipped: `ImageRiddleShareImage` (teal family; the riddle's own photo in a padded white frame with objectFit contain so no image crops; title; never the answer), `ogData.imageRiddleShare(id)` (public `/image-riddles/:id`, reads only title/imageUrl), og route `type=image-riddle` + clean path `/og/image-riddle/<id>.png` (degrades to the generic teal card, never the wrong-family joke card), new thin server route `app/image-riddles/[id]` with title/description/og:image + **self-canonical** (fixes `canonical -> /`), and both image-riddle ShareMenus (card + modal) now point at `/image-riddles/<id>`. Live: `/image-riddles/<id>` emits the riddle title, self-canonical + self og:url and `/og/image-riddle/<id>.png`; unknown ids render the generic teal card. Card in gui-test-screenshots/og-image-riddle-share01-8.png (visual gate pass). Owner-side FB debugger re-crawl of a real link still pending (platform caches). Originally: `https://pigzap.com/image-riddles?riddle=<id>` returned **Canonical URL = https://pigzap.com/** and `og:url = https://pigzap.com/`, so FB previews the generic home card. Steps:
...
**#10 games - DONE + LIVE-VERIFIED 2026-09-21.** WP2 built: new shared `lib/games-registry.ts` (single source for the hub page + og generator; per-game accent CSS gradients alongside the Tailwind pairs), `GameShareImage` template, og cases `type=game&slug=` / `type=games` / `type=play`, clean paths `/og/game/<slug>.png`, `/og/games.png`, `/og/play.png`; each of the 8 static games' index.html now carries og/twitter head tags pointing at its accent card (plain path, no query — BUG-063 rule); `/games` and `/play` emit their own og/twitter images instead of the home card. Live: all three clean paths 200 image/png (games.png byte-identical to the approved local render), `/games` + `/play` og:image tags present, static game HTML serves its og tags at `/games/<slug>/index.html` (the clean `/games/<slug>` 307 to index.html is the pre-existing BUG-015 redirect). Unknown slug degrades to the games-hub card.

**Deep-links (BUG-064 follow-up) - DONE + live 2026-09-21.** `lib/deep-links.ts` readers + client wiring: `/image-riddles?riddle=<id>` (legacy links) and `/image-riddles/<id>` (new share URLs) fetch the riddle and open its modal even off-page; `/jokes?joke=<id>` and `/jokes/<id>` flip that card open, jump to its page and scroll to it. Covered by `__tests__/deep-links.test.ts` (8 tests; full suite 561/561, backend 109/109). Live: `?riddle=<prodId>` 200. Note: the in-browser modal/flip pass was inconclusive this session — the shared dev server kept stalling the RSC stream (contended compiles); SSR HTML + metadata + unit tests all verified clean, and the wiring is client-side only (unchanged by the deploy).
...
- **Still open (follow-up):** RESOLVED 2026-09-21 — per-item preview cards (#8 image-riddle, #9 joke) are built and the params/paths now open the item (see SHARE-01 deep-links above).
