...
| Components | `RiddleCard.tsx` (reuses shared AnswerOptions/BubbleEmojiEffect), `RiddleReview`                                                                                                                                                                                                                          |
| Libs       | `lib/riddle-mcq-api.ts` (typed client incl. `getPublicLevelCounts`), `lib/riddle-scoring.ts` (single `isRiddleAnswerCorrect`), `lib/riddle-resume.ts` (two-key resume store), `hooks/use-riddle-play/*` (useRiddlePlay orchestration + useRiddleTimers clocks), `types/riddles.ts` (incl. adaptRiddleMcq) |
