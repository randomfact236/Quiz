...
function PracticeModeContent(): JSX.Element {
  ...
  const handleStartAllSubjectLevelWise = (level: Level) => {
    router.push(`/quiz/play?subject=all&chapter=all&level=${level.toLowerCase()}&mode=practice&type=challenge`);
  };

  const handleStartCompleteMix = () => {
    router.push(`/quiz/play?subject=all&chapter=all&level=all&mode=practice&type=challenge`);
  };
