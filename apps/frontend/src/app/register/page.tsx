...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Analytics plan §4b A7: client-side conversion anchor — this row
      // carries the device guestId the server-side `user_registered`
      // event lacks, giving the visitor→register funnel its join key.
      track('signup_completed', { method: 'password' }, { module: 'site' });
      router.push('/');
    } catch (err: unknown) {
      ...
      <div className="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
        <div className="bg-white dark:bg-slate-800 py-8 px-4 shadow sm:rounded-lg sm:px-10 border border-white/20">
            ...
            <div>
              ...
              <div className="mt-1 relative rounded-md shadow-sm">
                ...
                <input
                  ...
                  className="focus:ring-indigo-500 focus:border-indigo-500 block w-full pl-10 sm:text-sm border-slate-300 dark:border-slate-600 dark:bg-slate-700 dark:text-white rounded-md py-2.5 px-3 border"
          ...
          <div className="mt-5">
            ...
            <div className="mt-5">
              <GoogleLoginButton onLoginError={handleGoogleError} className="w-full" />
            </div>
          </div>
        </div>
      </div>
