...
export default function RiddleChallengePage(): JSX.Element {
  ...
  const handleStartChapterWise = (chapter: string, level: Level) => {
    router.push(`/riddles/play?chapter=${encodeURIComponent(chapter)}&level=${level.toLowerCase()}&mode=timer`);
  };

  const handleStartAllChapterLevelWise = (level: Level) => {
    router.push(`/riddles/play?chapter=all&level=${level.toLowerCase()}&mode=timer`);
  };

  const handleStartCompleteMix = () => {
    router.push(`/riddles/play?chapter=all&level=all&mode=timer`);
  };
                          ...
                          >
                            <div className="bg-gradient-to-r from-indigo-50 to-purple-50 rounded-xl p-4 border-2 border-indigo-200">
                              <p className="text-center text-sm text-gray-600 mb-3">
                                Select difficulty level for <span className="font-semibold text-indigo-600">
                                  {expandedChapter}
                                </span>
                              </p>
