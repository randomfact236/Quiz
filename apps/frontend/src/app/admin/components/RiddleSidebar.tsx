  ...
  return (
    <div>
      ...
      <button
        ...
        className={`w-full flex items-center gap-3 px-4 py-3 transition-colors text-sm font-medium ${isRiddlesActive && !activeChapter ? 'bg-blue-600 text-white' : 'text-gray-300 hover:bg-gray-800'}`}
      >
      ...
      {moduleExpanded && sidebarOpen && (
        <div className="px-2 mt-1">
          <div className="rounded-lg border border-transparent">
            {chapters.length === 0 ? (
              <div className="px-3 py-2 text-xs text-gray-500 dark:text-secondary-400 italic">
              ...
              chapters.map((chapter, index) => {
                ...
                return (
                  <div
                    ...
                    className={`group flex items-center gap-2 px-3 py-2 cursor-pointer will-change-transform border-t-2 border-b-2 rounded ${isActive ? 'bg-purple-600 text-white border-transparent' : isDragOver ? 'bg-purple-500/20 border-purple-500' : 'text-gray-300 hover:bg-gray-800 border-transparent text-sm'} ${isDragging ? 'opacity-50' : ''}`}
                    style={{
                      transition:
                  ...
                  >
                    <span
                      className={`flex items-center justify-center cursor-grab active:cursor-grabbing shrink-0 ${isActive ? 'text-white' : 'text-gray-500 dark:text-secondary-400 group-hover:text-gray-400 dark:hover:text-secondary-300'}`}
                    ...
                    <span
                      className={`text-xs px-1.5 py-0.5 rounded-full ${isActive ? 'bg-purple-700 text-white' : 'bg-gray-800 text-gray-500 dark:text-secondary-400'}`}
                    >
