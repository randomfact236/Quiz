  ...
  const findCol = (...names: string[]): number => {
    for (const name of names) {
      const idx = headers.findIndex(
        (h) => h.toLowerCase().replace(/\s+/g, ' ').trim() === name.toLowerCase()
      );
      if (idx !== -1) return idx;
    }
    return -1;
  ...
  for (let i = headerIndex + 1; i < lines.length; i++) {
    ...
    if (level === 'extreme') {
      if (!correctAnswerRaw || correctAnswerRaw.length === 0) {
        warnings.push(
          `Row ${i + 1}: extreme question has no answer text in "Correct Answer" column - use the actual answer text, not a letter. Skipping.`
        );
        continue;
      }
      ...
      if (['A', 'B', 'C', 'D'].includes(correctAnswerRaw.toUpperCase())) {
        warnings.push(
          `Row ${i + 1}: extreme question uses letter "${correctAnswerRaw}" but should use actual answer TEXT. Using as-is.`
        );
      }
      rows.push({
        questionText,
    ...
    if (level === 'easy') {
      if (isTFToken(optionA) || isTFToken(optionB)) {
        ...
        if (correctAnswerRaw.toUpperCase() === 'TRUE' || correctAnswerRaw.toUpperCase() === 'T') {
          ...
          correctAnswerRaw.toUpperCase() === 'FALSE' ||
          correctAnswerRaw.toUpperCase() === 'F'
        ) {
        ...
        rows.push({
          ...
          optionC: '',
          optionD: '',
          correctAnswerRaw: correctLetter,
          level,
          chapter,
        });
        continue;
    ...
    if (level === 'medium' && (hasC || hasD)) {
      warnings.push(
        `Row ${i + 1} (medium): medium level should have 2 options, but C/D are provided. Using first 2.`
      );
    }
    if (level === 'hard' && !hasC) {
      warnings.push(
        `Row ${i + 1} (hard): hard level requires 3 options (A,B,C). Missing Option C.`
      );
    }
    if (level === 'hard' && hasD) {
      warnings.push(
        `Row ${i + 1} (hard): hard level has 4 options, but should have 3. Using first 3.`
      );
    }
    if (level === 'expert' && (!hasC || !hasD)) {
      warnings.push(
        `Row ${i + 1} (expert): expert level requires 4 options (A,B,C,D). Missing options.`
      );
    }
    ...
    rows.push({
      ...
      optionC,
      optionD,
      correctAnswerRaw: correctLetter || 'A',
      level,
      chapter,
    });
  }
  ...
  const rawPayload = rows.map((row, idx): QuestionPayload | null => {
    ...
    if (!chapterId) {
      ...
      if (availableChapters.length > 0 && availableChapters[0]) {
        ...
        warnings.push(
          `Row ${idx + 1}: Chapter "${row.chapter}" not found, using first available chapter`
        );
      } else {
