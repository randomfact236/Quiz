...
function QuizContent(): JSX.Element {
  ...
  // TASK-27: shared question deep link (uuid of the shared question)
  const sharedQuestionId = searchParams?.get('qid') || null;
        ...
        <div className="mx-auto w-full max-w-5xl flex flex-col flex-1 justify-center">
          ...
          {shareMenuOpen && quiz.currentQuestion && (
            <ShareMenu
              ...
              url={`${SITE_URL}/quiz-mcq/play?subject=${subject}&chapter=${encodeURIComponent(chapter)}&level=${level}&mode=${mode}&shared=true&total=${quiz.totalQuestions}&qid=${quiz.currentQuestion.id}`}
