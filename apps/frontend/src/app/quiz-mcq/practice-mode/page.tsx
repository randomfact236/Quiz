...
export default function PracticeModePage(): JSX.Element {
  ...
  const handleStartSubjectWise = (subject: string, level: Level) => {
    router.push(
      `/quiz-mcq/play?subject=${subject}&chapter=all&level=${level.toLowerCase()}&mode=practice&type=challenge`
    );
  };

  const handleStartAllSubjectLevelWise = (level: Level) => {
    router.push(
      `/quiz-mcq/play?subject=all&chapter=all&level=${level.toLowerCase()}&mode=practice&type=challenge`
    );
  };

  const handleStartCompleteMix = () => {
    router.push(`/quiz-mcq/play?subject=all&chapter=all&level=all&mode=practice&type=challenge`);
  };
                        ...
                        {row.map((subject) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`text-xs mt-1 ${isExpanded ? 'text-white/80' : totalQuestions > 0 ? 'text-gray-500' : 'text-orange-500'}`}
                              >
                                {totalQuestions > 0
                                  ? isHydrated
                                    ? `${totalQuestions} Qs`
            ...
            >
              <div className="flex items-center gap-4">
                ...
                <div className="text-left">
                  ...
                  <span className="text-sm opacity-90">
                    All subjects, all levels - Ultimate practice!
                  </span>
                </div>
              </div>
              {completeMixOpen ? (
