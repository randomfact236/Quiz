...
export default function TimerChallengePage(): JSX.Element {
  ...
  const handleStartSubjectWise = (subject: string, level: Level) => {
    router.push(
      `/quiz-mcq/play?subject=${subject}&chapter=all&level=${level.toLowerCase()}&mode=timer&type=challenge`
    );
  };

  const handleStartAllSubjectLevelWise = (level: Level) => {
    router.push(
      `/quiz-mcq/play?subject=all&chapter=all&level=${level.toLowerCase()}&mode=timer&type=challenge`
    );
  };

  const handleStartCompleteMix = () => {
    router.push(`/quiz-mcq/play?subject=all&chapter=all&level=all&mode=timer&type=challenge`);
  };
                        ...
                        {row.map((subject) => {
                          ...
                          return (
                            ...
                            >
                              ...
                              <span
                                className={`text-xs mt-1 ${isExpanded ? 'text-white/80' : 'text-gray-500'}`}
                              >
