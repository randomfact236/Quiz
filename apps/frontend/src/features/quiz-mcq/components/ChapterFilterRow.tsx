...
export function ChapterFilterRow({
  ...
  return (
    <div
      className={`flex items-center gap-1 rounded-lg border px-2 py-1 transition-colors ${isSelected ? 'bg-indigo-500 border-indigo-600' : 'bg-white border-gray-300 hover:bg-indigo-50'}`}
    >
      <button
        onClick={onSelect}
      ...
      >
        {chapter.name}
        <span className={`ml-1 text-xs ${isSelected ? 'text-indigo-200' : 'text-gray-400'}`}>
          ({count})
        </span>
      </button>
      <button
