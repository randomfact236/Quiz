...
import type { QuizSubject, QuizChapter, FilterCountsResponse } from '@/lib/quiz-mcq-api';
      ...
      >
        <div className="flex flex-wrap items-center gap-2">
          ...
          {sortedSubjects.map((subject) => {
            const subjectCount =
              filterCounts?.subjects?.find((s) => s.name === subject.name)?.count || 0;
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {filteredChapters.map((chapter) => {
            ...
            const chapterCount =
              filterCounts?.chapterCounts?.find((c) => c.id === chapter.id)?.count || 0;
            return (
              <ChapterFilterRow
                key={chapter.id}
                chapter={chapter}
