            ...
            {getPageNumbers(page, totalPages).map((pageNum, index) =>
              pageNum === '...' ? (
                ...
                <button
                  ...
                  disabled={isFetching}
                  className={`px-3 py-1 border rounded disabled:opacity-50 disabled:cursor-not-allowed ${page === pageNum ? 'bg-blue-500 text-white border-blue-500' : 'hover:bg-gray-100 dark:hover:bg-secondary-800 dark:hover:bg-gray-700'}`}
                >
                  {pageNum}
                </button>
              )
