/**
 * ============================================================================
 * config.js — Hurdle Runner (flags + strings, host-overridable)
 * ============================================================================
 * Rev 2 pattern (plan/games/05-continuous-runner.md §12): environment flags
 * and per-locale UI strings in one host-overridable place:
 *
 *   defaults  ←  window.__HURDLE_RUNNER_CONFIG__ (host page / WebView)
 *             ←  ?locale=fr URL param (testing)
 *
 * Physics/tuning constants (§2 tables) stay in core.js — the spec of record.
...
function readHostOverride() {
  try {
    if (typeof window !== 'undefined' && window.__HURDLE_RUNNER_CONFIG__) {
      return window.__HURDLE_RUNNER_CONFIG__;
    }
  } catch {
