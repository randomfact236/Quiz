/**
 * ============================================================================
 * storage.js — Flying Snake (guarded persistence facade)
 * ============================================================================
 * Rev 2 pattern (plan/games/06-flying-snake.md §12): one versioned save
 ...
 * N/A by design (runs are seconds; death = instant restart).
 *
 * Legacy layout (pre-Rev 2, migrated on first write, then removed):
 *   game:flying-snake:best   → { score }
 *   game:flying-snake:prefs  → { muted }
 *
 * Versioned layout:
 *   game:flying-snake:save   → { version: 1, best: { score }, prefs: { muted } }
 * ============================================================================
 */

import { BEST_KEY, PREFS_KEY } from './core.js';

export const SAVE_KEY = 'game:flying-snake:save';
...
function loadSave() {
  ...
  const save = {
    version: SAVE_VERSION,
    best:
      legacyBest && typeof legacyBest === 'object' && typeof legacyBest.score === 'number'
        ? { score: legacyBest.score }
        : { score: 0 },
...
/** `{ score } | null` — null before the first run. */
export function getBest() {
  const best = loadSave().best;
  return best.score > 0 ? best : null;
}

/** Stores `score` when it beats the record; returns true exactly then. */
export function saveBest(score) {
  if (typeof score !== 'number' || score <= 0) return false;
  const save = loadSave();
  if (save.best.score >= score) return false;
  save.best = { score };
  persist(save);
  return true;
}
