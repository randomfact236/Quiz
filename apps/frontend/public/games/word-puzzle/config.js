...
const DEFAULT_CONFIG = {
  locale: 'en',
  /** P3 daily puzzle — the deterministic generator ships; the mode is gated. */
  dailyEnabled: false,
  strings: {
    en: {
...
export function resolveConfig(base, ...overrides) {
  ...
  for (const layer of [base, ...overrides]) {
    ...
    if (typeof layer.dailyEnabled === 'boolean') out.dailyEnabled = layer.dailyEnabled;
...
function readUrlOverrides() {
  try {
    ...
    if (daily === '1' || daily === '0') override.dailyEnabled = daily === '1';
