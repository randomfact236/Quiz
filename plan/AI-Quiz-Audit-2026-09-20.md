...
- [x] H1 public reads: strip answer keys (`answer`, `correctAnswer`, `correctLetter`, `options`) from public DTOs - DONE 2026-09-23 (phase 2c complete; was QA HARD-02): phases 1/2a/2b shipped 2026-09-21 (below). Phase 2c: all three graders (quiz/riddle/image-riddle answers-check) return the VERDICT ONLY — they previously echoed the answer key on every call; grading is by OPTION TEXT (shuffle-proof — the BUG-041 serve-shuffle remaps the served letter per response, so letter-vs-stored-letter grading was wrong whenever the correct value moved slots); throttled reveal endpoints serve the key ONLY for post-session review/give-up; play reads stripped (quiz random/mixed/by-level/by-id, riddle subject/mixed/random/by-id, image-riddle search ships answerLength). Frontend quiz+riddle engines grade via the server (verdicts persist in resume snapshots, old snapshots keep embedded keys as fallback); image-riddle grades server-side, reveals on correct guess or give-up. Verified: backend 129/129 + tsc; frontend 561/561 + tsc; Playwright quiz 7/7, riddle 4/4; image-riddle API probes.
...
- [x] H6 unbind prod app ports (or loopback-bind) + restrict origin firewall to Cloudflare IPs - FIXED 2026-09-22/23 (was QA NOW-01): ufw enabled default-deny; the 80/443 block lives in the iptables DOCKER-USER chain (Docker's DNAT bypasses ufw INPUT — proven live); idempotent `/usr/local/sbin/cf-docker-firewall.sh` + systemd unit re-applies at boot; post-fix probes: pigzap.com + api.pigzap.com via Cloudflare = 200, direct-origin = connection timeout. ✅ Dokploy :3000 closed 2026-09-23: owner-IP ACCEPT rules from `/etc/dokploy-allow-ips.list` + :3000 DROP for everyone else (IPv6 dropped), same systemd unit; reopen one-liner in QA-FINDINGS history / runbook.
- [x] H8 add a CSP and (at minimum) plan token-storage hardening - PARTIAL 2026-09-21: baseline CSP shipped (self + GTM; unsafe-inline while inline scripts are un-nonced); token storage hardening remains. UPDATE 2026-09-22 (was QA HARD-01): the real CSP gap — the eight static games served NO CSP — closed via middleware decorating every `/games/*` response with the strictest policy on the site (`script-src 'self'`, no remote anything, connect-src same-origin + API origin; two inline theme-loader hashes). Full nonces assessed and NOT pursued (incompatible with ISR/static games/Next bootstrap); 'unsafe-inline' on app pages = documented residual; HttpOnly refresh cookies = owner decision, parked.
...
- [x] SEC-10/SEC-12 bind anonymous writes to a server-signed guest token - FIXED 2026-09-22 (was QA HARD-03): `token = HMAC-SHA256(GUEST_TOKEN_SECRET‖JWT_SECRET, guestId)` via guest-token.service; `POST /guest-users/token` (throttled 10/min) signs server-minted `srv_…` ids AND existing legacy `guest_…` ids as-is (migration path keeps every existing visitor's likes/comments matching); GuestTokenGuard enforces the pair on like, comment create + delete-own, guest activity heartbeat, and login merge; frontend `ensureGuestToken()` caches + attaches on all four write helpers, 403 auto-re-issues once and retries. Scope notes: comment `flag` has no identity to bind (idempotent by comment id); analytics ingest keeps unsigned guestId (read-only attribution); duels' game-scoped guestId endpoints get the binding in a follow-up pass. Verified constant-time; guest-token spec 6/6; frontend 561/561; backend suite green.
...
- [x] BE-09 run the CSV leakage/ambiguity repairs and re-audit - DONE 2026-09-22 (was QA TASK-03): `scripts/repair-be09-csv.py` + `repair-be09-db.sql`, 10 rows fixed on both sides, re-audit clean; residue (60 over-long riddles + bakery-alibi contradiction) → NOW-07 in QA-FINDINGS.
...
- [x] OPS-19 off-box backup replication + documented restore drills - DONE 2026-09-22 (was QA NOW-04): rclone remote `r2` confirmed copying nightly dumps to R2 (objects present 2026-09-22); restore drill PASS — `quiz_db_20260922_033002.sql.gz` restored into a throwaway postgres:18 container, counts matched prod exactly (questions 11,541 / riddle_mcqs 3,000 / users 1 / comments 3 / question_likes 6); drill finding fixed: backup.sh now dumps with `--no-owner --no-privileges` (had emitted 38 `OWNER TO` errors in a fresh container). Drill logged in `/opt/quiz-backups/README.md`.
- [x] OPS-21 uptime/error alerting + incident/rollback runbook - WIRED LIVE 2026-09-22 (was QA NOW-05): `/usr/local/bin/quiz-uptime.sh` on a 1-minute cron checks https://pigzap.com/ + the API summary endpoint, counts consecutive failures (state in `/opt/quiz-alerts/`), fires `{text:…}` JSON webhooks on failure AND recovery, and dead-man-pings a heartbeat URL on every clean run. Remaining owner step (tracked as NOW-05): put a real channel (healthchecks.io / Discord / Telegram webhook) into `/opt/quiz-alerts/alert.env` as `HEARTBEAT_URL=` / `ALERT_WEBHOOK=`.
- [ ] Cosmetics §5
