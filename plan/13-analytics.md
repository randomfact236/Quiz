...
| `app/admin/components/AnalyticsSection.tsx` | Tabbed dark dashboard: Overview / Quiz MCQ / Riddle MCQ / Image Riddles / Dad Jokes / Users / Audience & Geo / Retention / Raw Events; range selector 24h/7d/30d/90d; per-tab CSV export (client-side); `?section=analytics&tab=` deep links; module tabs include a "Sessions abandoned" funnel row                                                                                                           |
| `app/admin/components/analytics/`           | Tab implementations (`tabs.tsx`), `primitives.tsx` (stat cards, CSS bars), `types.ts`, `csv.ts`                                                                                                                                                                                                                                                                                                               |
| `app/admin/components/EventsBrowser.tsx`    | Raw events tab: eventName filter (free text + datalist of known names), module, date range, actor (userId/guestId/sessionId) filters; paginated table with actor + per-row property details                                                                                                                                                                                                                   |
...
- [ ] **B1. Funnel + conversion views** — endpoints + UI for visit→register→first-quiz→return; `signup_completed` (A7, done 2026-09-05) provides the guest-attributed conversion anchor, so the guest→register step is queryable today; full identity stitching for post-registration events still needs work (P2 carry-over).
