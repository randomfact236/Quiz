...
- **2026-08-30 — Feature 10 Landing & Shared UI: complete.** P1: StatsSection now renders site-wide stats from `GET /analytics/summary` (local-stats fallback); topic cards verified live-data-driven. P2: /play added to the shared nav config (all menus); landing SEO verified via layout + sitemap; health response shapes documented in the plan (liveness/readiness variants already existed). P3: reduced-motion respected on BubbleBackground; ui-kit tests accepted as deferred-by-design.
...
- **RESOLVED 2026-09-03:** dev Postgres restored by starting the project's own `ai-quiz-postgres`/`ai-quiz-redis` containers (credentials match `apps/backend/.env`; the `backend_postgres_data` volume held the fully current schema — the earlier "stale data" concern was unfounded, the container was the live dev DB). All five pending migrations applied (`1789000000000`–`1789400000000`), backend rebuilt (it had been serving a pre-F11 dist) and restarted, frontend dev server back on :3010. Live probes passed for every pending feature: newsletter (subscribe/idempotent/honeypot/unsubscribe/400, correct DB rows incl. honeypot discard), settings/public levelTimers, image-riddle engage, quiz session create + history. Probe rows cleaned up.
