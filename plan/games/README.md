...
> `2d games plan.md` (repo root); 08 added 2026-09-12 by owner request).
...
| 8   | Memory Quiz      | `08-memory-quiz.md`       | **Built + upgraded** — P0–P2 complete in `public/games/memory-quiz/` (config/storage/audio/packs modules; where + missing questions, streak scoring, daily board, history + top-% badge, share chain); jest + core.test.html green. Pack deviation recorded: 14 items per pack (plan listed 10) — level-aware lintPacks enforces the spare budget. 2026-09-12 post-play pass: items hide when the question phase begins (truth reveals on feedback) and a 🎯 Level chip joined the HUD. **2026-09-12 extension upgrade (`08-memory-quiz-upgrade.md`, Phases A–C built):** 30-level campaign (bands of 10: 4/6/8 blocks, fully-filled grids) with stars/unlocks/level picker, endless generator, question-type registry (where/missing/swap/odd-one), mode registry (endless/zen/hard/kids/time-attack), save v2 with durable v1 migration; 38 jest tests + 37-assertion harness + 38-check E2E green. Phase D extras (canvas grids, photo packs, leaderboards) deferred | done      |
