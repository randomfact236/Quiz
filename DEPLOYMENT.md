...
- PostgreSQL connection uses `ssl: false` inside the Docker network (traffic never leaves the host; set `DB_SSL=true` if you move to a managed/remote Postgres)
- User registration is DISABLED - admin accounts via CLI only
- Registration endpoint removed from auth controller
- Reverse proxy: **HTTP→HTTPS redirect must be on** (Dokploy/Traefik does this per-domain — verify the certificate issue mode is not "skip"). HSTS is served by the backend via helmet; the proxy should also send it for frontend traffic
- Set `TRUST_PROXY=true` on the backend so rate limiting and brute-force lockout see real client IPs behind the proxy
- Redis requires a password in production (`REDIS_PASSWORD` in the compose `.env`)
- Refresh tokens are stored hashed and rotated on every use (replay fails the lookup)
...
On the VPS, add a cron job (`crontab -e`) that dumps nightly and keeps 14 days:

```bash
0 3 * * * docker exec quiz-postgres pg_dump -U aiquiz aiquiz | gzip > /var/backups/aiquiz/backup_$(date +\%Y\%m\%d).sql.gz && find /var/backups/aiquiz -name 'backup_*.sql.gz' -mtime +14 -delete
```

Create the directory first (`mkdir -p /var/backups/aiquiz`). Copy the
`backup_*.sql.gz` files off the box periodically — backups stored only on the
same VPS do not survive a lost disk.
...
# or for gzipped dumps:
gunzip -c backup_YYYYMMDD.sql.gz | docker exec -i quiz-postgres psql -U aiquiz -d aiquiz
```
