...
powershell -Command "try { $r = Invoke-WebRequest -Uri 'http://localhost:3010' -TimeoutSec 3 -UseBasicParsing; exit 0 } catch { exit 1 }"
if %ERRORLEVEL% == 0 (
...
powershell -Command "$urls=@('http://127.0.0.1:3010','http://localhost:3010','http://192.168.1.65:3010'); foreach($u in $urls){try{$r=Invoke-WebRequest -Uri $u -TimeoutSec 3 -UseBasicParsing; Write-Host \"    ? $u - OK\" -ForegroundColor Green}catch{Write-Host \"    ? $u - Failed\" -ForegroundColor Red}}"
...
echo     Launching: http://localhost:3010
echo.
echo ------------------------------------------
echo If browser shows error, TRY THESE URLs:
echo   1. http://127.0.0.1:3010
echo   2. http://192.168.1.65:3010
echo ------------------------------------------
echo.
