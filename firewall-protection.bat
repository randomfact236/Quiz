...
netsh advfirewall firewall add rule name="AIQuiz_Block_External_3012" dir=in action=block protocol=tcp localport=3012 remoteip=internet profile=any >nul 2>&1
if %errorLevel% == 0 (echo [OK] External access blocked for port 3012) else (echo [INFO] Rule for 3012 already exists)
...
netsh advfirewall firewall add rule name="AIQuiz_Allow_Local_3012" dir=in action=allow protocol=tcp localport=3012 remoteip=localsubnet profile=any >nul 2>&1
...
echo.
echo Summary:
echo - Ports 3010, 3012, 5432, 6379 protected
